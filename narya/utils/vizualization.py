...
def merge_template(img, warped_template):
    # Convert image data type to float for accurate blending
    img_float = img.astype("float32")
    warped_float = warped_template.astype("float32")
    
    # Calculate valid index where warped_template has non-zero values
    valid_index = warped_float[:, :, 0] > 0.0
    
    # Blend images using weighted average
    overlay = (img_float * 0.95 + warped_float * 0.05)[valid_index]
    # overlay = (img_float * 0.05 + warped_float * 0.95)[valid_index]
    
    # Copy the original image to maintain the original content
    new_image = np.copy(img)
    
    # Replace pixels at valid_index with the overlay
    new_image[valid_index] = overlay.astype("uint8")
